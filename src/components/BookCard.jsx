...
function BookCard({ book, index }) {
  return (
    <Link to={`/book/${index}`} className="transform hover:scale-105 transition-transform duration-300 ease-in-out">
      <div className="bg-black p-4 rounded-lg shadow-lg hover:shadow-2xl overflow-hidden transition-transform transform hover:scale-105">
        ...
        <p className="text-sm text-red-500">{book.author}</p>
        <p className="text-xs text-yellow-600 mt-2">{book.genre}</p>
      </div>
    </Link>
  );
}
